...
Репозитории (`lib/data/*_repository.dart` поверх drift/SQLite, write-through кэш) → Riverpod-провайдеры (`state/providers.dart`: операции, категории, счета, бюджеты, цели, правила, регулярные + производные `monthStatsProvider`, `netWorthProvider`, `rubAmountProvider`) → экраны (`screens/`) → переиспользуемые виджеты (`widgets/`).
...
- `lib/data/database.dart` — схема drift (`NumoDatabase`, 9 таблиц); после её изменения запускать `dart run build_runner build --delete-conflicting-outputs` и поднимать `schemaVersion` с миграцией.
...
- Общие счета (ADR-0014): `lib/data/shared_sync.dart` (файлы участников в общей папке), `lib/data/members_repository.dart` (справочник людей), слияние — в `mergeAll` соответствующих репозиториев.
...
- Секреты пользователя (API-ключ LLM) — только через `lib/data/secret_store.dart` (системный keychain с фолбэком, ADR-0014), НЕ в shared_preferences напрямую.
...
- **Инициализация идёт после `runApp`**: `main()` только поднимает `NumoBootstrap`, хранилища открывает `lib/data/startup.dart` под уже показанным окном. Не переносить открытие базы обратно в `main()` — заминка там оставляет процесс без окна, и пользователю нечего показать.
- **Шаги миграции идемпотентны**: колонки добавляются через `_addColumn`, таблицы — через `_createTable` (проверяют `PRAGMA table_info` и `sqlite_master`). Прерванный запуск может оставить `ALTER TABLE` применённым без записи новой версии схемы — тогда повтор того же шага роняет приложение на «duplicate column name».
